...
var arrIndexOf = function(anArray, searchElement) {
...
var itemIsArray = function(anObject) {
...
JSONPath.prototype.evaluate = function (expr, json, callback, otherTypeCallback) {
    ...
    if (!expr || !json || arrIndexOf(allowedResultTypes, this.currResultType) === -1) {
        return;
    }
    this._obj = json;
    ...
    if (result.length === 1 && !wrap && !itemIsArray(result[0].value)) {
        return this._getPreferredOutput(result[0]);
    }
    return result.reduce(function (result, ea) {
        var valOrPath = self._getPreferredOutput(ea);
        if (flatten && itemIsArray(valOrPath)) {
