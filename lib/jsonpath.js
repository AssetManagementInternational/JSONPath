 ...
 */
/*
THIS IS A COMMENT THAT SHOULD BE REMOVED .. testing notification
*/
var module;
(function (glbl, require) {'use strict';

	var indexOf = function (anArray, searchElement) {
		for (var i = 0; i < anArray.length; i++) {
			if (anArray[i] === searchElement) {
				return i;
			}
		}
		return -1;
	...
	var isArray = function (anObject) {
		return anObject !== undefined && anObject !== null && anObject.push && anObject.pop;
	}
	...
	var reduce = function (arr, callbackfn, initialValue) {
		var O = Object(arr),
			lenValue = O.length,
			len = lenValue >>> 0,
			k,
...
var vm = isNode
    ? require('vm') : {
        runInNewContext: function (expr, context) {
            var keys = objectKeys(context);
            var funcs = [];
            moveToAnotherArray(keys, funcs, function (key) {
                return typeof context[key] === 'function';
            });
            var code = reduce(funcs, function (s, func) {
                return 'var ' + func + '=' + context[func].toString() + ';' + s;
            }, '');
            code += reduce(keys, function (s, vr) {
...
function JSONPath (opts, expr, obj, callback, otherTypeCallback, exec) {
    if (!(this instanceof JSONPath)) {
        try {
            return new JSONPath(opts, expr, obj, callback, otherTypeCallback, exec);
        }
        catch (e) {
            if (!e.avoidNew) {
                throw e;
	...
	this.exec = opts.exec || exec || function(){ throw Error('No callback for exec parameter.');};
...
JSONPath.prototype.evaluate = function (expr, json, callback, otherTypeCallback) {
    ...
    if (!expr || !json || indexOf(allowedResultTypes, this.currResultType) === -1) {
        return;
    }
    this._obj = json;
    ...
    result = filter(result, function (ea) {return ea && !ea.isParentSelector;});

    if (!result.length) {return wrap ? [] : undefined;}
    if (result.length === 1 && !wrap && !isArray(result[0].value)) {
        return this._getPreferredOutput(result[0]);
    }
    return reduce(result, function (result, ea) {
        var valOrPath = self._getPreferredOutput(ea);
        if (flatten && isArray(valOrPath)) {
...
JSONPath.prototype._trace = function (expr, val, path, parent, parentPropName, callback) {
    ...
    if (val && Object.prototype.hasOwnProperty.call(val, loc)) { // simple case--directly follow property
        ...
        addRet(this._trace(unshift(this._exec(loc, val), x), val, path, parent, parentPropName, callback));
	}
    ...
    // ancestor object of val
        ...
        this._walk(loc, x, val, path, parent, parentPropName, callback, function (m, l, x, v, p, par, pr, cb) {
            if (self._exec(l.substring(1), v[m])) {
                addRet(self._trace(unshift(m, x), v, p, par, pr, cb));
            }
        });
	}
    ...
    // current val object
    return reduce(ret, function (all, ea) {
...
JSONPath.prototype._exec = function(obj, _v){
	return this.exec(JSON.parse(obj), _v);
}
...
JSONPath.toPathArray = function (expr) {
    ...
    var exprList = map(normalized.split(';'), function (expr) {
