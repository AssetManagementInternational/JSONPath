...
function JSONPath (opts, expr, obj, callback, otherTypeCallback, functionMap) {
    if (!(this instanceof JSONPath)) {
        try {
            return new JSONPath(opts, expr, obj, callback, otherTypeCallback, functionMap);
        }
        catch (e) {
            if (!e.avoidNew) {
                throw e;
	...
	this.functionMap = opts.functionMap || functionMap || {};
...
JSONPath.prototype._trace = function (expr, val, path, parent, parentPropName, callback) {
    ...
    if (val && Object.prototype.hasOwnProperty.call(val, loc)) { // simple case--directly follow property
        ...
        addRet(this._trace(unshift(this._exec(loc.replace(/^\{(.*?)\}$/, '$1'), val), x), val, path, parent, parentPropName, callback));
	}
...
JSONPath.prototype._exec = function(fname, _v){
	if (typeof(this.functionMap[fname]) === 'function'){
		return this.functionMap[fname](_v);
	}
	
	 throw new Error('No function named \''+fname+'\'.');
}
...
}(typeof require === 'undefined' ? null : require, self || this));
